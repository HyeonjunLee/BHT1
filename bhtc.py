...
def transient_heat_conduction_composite_variable_properties(T_outside, inner_diameter_mm, steel_thickness_mm, d_chrome_mm,
    ...
    Returns:
      ...
      T_record: Recorded temperature distribution (°C), shape = (num_snapshots, N)
    ...
    # Create a uniform radial grid from r1 to r2
    ...
    # For chrome, density is 7190 kg/m³
    ...
    for n in range(num_steps + 1):
        ...
        # Material properties for node 0 (chrome)
        k0 = thermal_conductivity_chrome(T[0])
        cp0 = specific_heat_chrome(T[0])
        # For node 1, decide based on its material
        if is_chrome[1]:
        ...
        for i in range(1, N - 1):
            # Determine material properties at node i
            if is_chrome[i]:
            ...
            # Effective conductivities at interfaces
            k_im_eff = 0.5 * (k_im + k_i)
            k_ip_eff = 0.5 * (k_i + k_ip)
            # Radial positions at interfaces
            r_im = 0.5 * (r[i] + r[i - 1])
            r_ip = 0.5 * (r[i] + r[i + 1])
            # Finite difference flux term in cylindrical coordinates
...
N = 300                     # Number of radial grid points (더 세밀한 해상도)
num_snapshots = 50            # Snapshot 개수 (linspace로 균일하게 간격 결정)

# Run simulation
r, T_record, times, T_air_record = transient_heat_conduction_composite_variable_properties(
...
# Convert radial coordinate from m to mm for plotting
r_mm = r * 1000
...
ax.set_title('Transient Temperature Distribution in Composite Cylinder', fontsize=16)
ax.grid(True)
